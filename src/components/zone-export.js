export class ZoneExport extends HTMLElement {
  connectedCallback() {
    ...
    this.dialog.querySelector('.export-output').setAttribute('aria-label', 'Generated export output');
    this.dialog.querySelector('.copy-button').setAttribute('aria-describedby', 'copy-status');
    this.dialog.querySelector('.copy-button').insertAdjacentHTML('afterend', '<span id="copy-status" class="copy-status" role="status" aria-live="polite"></span>');
    ...
    this.querySelector('.copy-button').addEventListener('click', async () => { const status = this.querySelector('#copy-status'); try { await navigator.clipboard.writeText(this.querySelector('.export-output').value); status.textContent = 'Copied to clipboard'; } catch { this.querySelector('.export-output').select(); status.textContent = 'Clipboard unavailable. Select the text to copy.'; } setTimeout(() => { status.textContent = ''; }, 1800); });
  }

  open(trigger) { this.lastTrigger = trigger; if (!this.dialog.open) { this.dialog.showModal(); this.dialog.querySelector('[data-key="zoneName"]').focus(); } }
